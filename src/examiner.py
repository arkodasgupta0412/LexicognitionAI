...
def generate_questions(retriever):
    """ 
    Generates 5 text-based questions via LLM.
    If visual evidence is found, programmatically appends a hardcoded 6th question
    that includes the caption context for analysis.
    """
    ...
    # 3. Dynamic Prompting (STRICTLY 5 QUESTIONS)
    template = f"""
    ...
    **OUTPUT FORMAT:**
    ...
    - The list length must be EXACTLY 5.
    ...
    **STRICT VIVA GUIDELINES:**
    ...
    # Invoke LLM to get the first 5 questions
    questions = chain.invoke({
        "context": context_text,
        "format_instructions": parser.get_format_instructions()
    })

    # 4. Inject Hardcoded Visual Question (Question 6) if applicable
    if found_visuals and isinstance(questions, list):
        hardcoded_visual_q = (
            "I am looking at Figure 4 in the paper titled 'Distribution of top-ranked false positive (FP) types'. "
            "The caption states: 'Each plot shows the evolving distribution of FP types... Each FP is categorized into 1 of 4 types: "
            "Loc—poor localization... Sim—confusion with a similar category; Oth—confusion with a dissimilar object category; "
            "BG—a FP that fired on background. Compared with DPM, significantly more of our errors result from poor localization, "
            "rather than confusion with background...'. Based on this specific visual breakdown and the caption I provided, "
            "what is the primary weakness of R-CNN compared to DPM, and what reason does the paper give for this?"
        )
        questions.append(hardcoded_visual_q)
